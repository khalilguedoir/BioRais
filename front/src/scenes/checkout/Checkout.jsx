...
const Checkout = () => {
  ...
  async function request_orders(values) {
    const products_list = []
    let orders_id = await cart.forEach(async (item) => {
      const request_order = {
        "data": {
          produit: (item.attributes.name).toString(),
          quantity: (item.count).toString(),
          somme: (item.attributes.price * item.count).toString()
        }
      }
      ...
      const response = await fetch("http://localhost:1337/api/orders", {
        ...
        body: JSON.stringify(request_order),
      })
      if (response.status == 200) {
        const res = await response.json()
        console.log("order id :", res.data.id)
        products_list.push(res.data.id)
      }

    })

    makeOrder(values, products_list)

    console.log("Done" + products_list)
    return products_list;
  }

  async function makeOrder(values, orders) {
    console.log({ orders });
    const requestBody = {
      "data": {
    ...
    const response = await fetch("http://localhost:1337/api/client-orders", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    console.log({ res });
    if (res) {
      setConfirmed(res.data.attributes.products)
      handleClickOpen()
    }
  }
              ...
              <Box display="flex" justifyContent="space-between" gap="50px">
                ...
                {activeStep <= 2 &&
                  (
                    ...
                    >
                      {isFirstStep ? "Proceed to checkout" : isSecondStep ? "CONFIRMER LA COMMANDE" : undefined}
                    </Button>
                  )
                }
